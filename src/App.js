import { Card, Button, FormGroup, InputGroup, H2, TextArea, Intent, HTMLSelect, Toaster, Position } from "@blueprintjs/core";
...
function App() {
  ...
  return (
    <Card elevation="1">
      <Toaster ref={toastRef} autoFocus={false} canEscapeKeyClear={true} position={Position.TOP} usePortal={true} />
      ...
      <form className="notification-form" onSubmit={formSubmitHandler}>
        ...
        <FormGroup label="Send To" labelFor="data">
          <HTMLSelect fill options={recipients} value={to} onChange={(e) => setTo(e.target.value)} />
        </FormGroup>
        <Button intent="primary" fill type="submit" text={isSubmitting ? "Sending" : "Send"} />
      </form>
    </Card>
  );
}
